...
public class NamespaceUnlockAspect {
    ...
    //create item 方法上标记了该注解并且方法参数上有,这里的拦截主要是controller层的
    ...
    public static void main(String[] args) {
        ...
        //这能
        ImmutableMap<Integer, Student> immutableMap = Maps.uniqueIndex(geoStudents, new Function<Student, Integer>() {
        ...
        System.out.println(difference.areEqual()); //是否全等
        System.out.println(difference.entriesDiffering().toString()); //key相同，value不同
        System.out.println(difference.entriesInCommon().toString()); //获取公共的key value都要相同
        System.out.println(difference.entriesOnlyOnLeft().toString()); //仅仅左边有 key value都不一样
        System.out.println(difference.entriesOnlyOnRight().toString()); //仅仅右边有
    }
